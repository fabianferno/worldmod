...
- **Two confidential workflows ship here.** `episode-validator/` confidentializes the episode *validator* (secret acceptance threshold → pass/fail verdict). `utility-oracle/` confidentializes *utility settlement* (§8.3): a secret weighting policy and the raw per-contributor deltas stay in the enclave, only the normalised basis-point shares egress, and the DON co-signs them into `ChainlinkUtilityOracleConsumer` — now `BountyEscrow`'s registered `oracle` (via `setOracle`). This replaces §8.3's trusted UtilityOracle operator. *Multiple independent trainers reaching consensus* remains roadmap; what's solved is removing the single trusted operator.
